...
var _ = Describe(" Dns", func() {
	...
	var DnsResourceData *schema.ResourceData
	var model_json = `
                         {
   "cnode_ids": [
      1,
   ...
   "vip_vlan": 100
	...
	DnsResource := resources.ResourceDns()
	ReadContext = DnsResource.ReadContext
	DeleteContext = DnsResource.DeleteContext
	CreateContext = DnsResource.CreateContext
	UpdateContext = DnsResource.UpdateContext
	Importer = *DnsResource.Importer
	//	ResourceSchema = DnsResource.Schema

	BeforeEach(func() {
		DnsResourceData = DnsResource.TestResourceData()
		DnsResourceData.SetId("100")
	...
	Describe("Validating Resource Read Context", func() {
		Context("Read Data into a ResourceData", func() {
			It("Resource:Dns ,Reads Data", func() {
				...
				d := ReadContext(ctx, DnsResourceData, client)
				Expect(d).To(BeNil())
				attributes := DnsResourceData.State().Attributes
				for k, v := range z {
	...
	Describe("Validating Resource Delete Context", func() {
		Context("Delete A resource", func() {
			It("Resource:Dns ,Deletes the resource", func() {
				var ctx context.Context = context.Background()
				var output bytes.Buffer
				...
				e := client.Start()
				Expect(e).To(BeNil())
				d := DeleteContext(ctx, DnsResourceData, client)
				Expect(d).To(BeNil())
			})
		})
	},
	...
	Describe("Validating Resource Creation Context", func() {
		Context("Create A resource", func() {
			It("Resource:Dns ,Creates the resource", func() {
				var ctx context.Context = context.Background()
				var output bytes.Buffer
				...
				resources.ResourceDnsReadStructIntoSchema(ctx, resource, DnsResourceData)
				DnsResourceData.SetId("100")
				d := CreateContext(ctx, DnsResourceData, client)
				Expect(d).To(BeNil())
	...
	Describe("Validating Resource Update Context", func() {
		Context("Update A resource", func() {
			It("Resource:Dns ,Update the resource", func() {
				var ctx context.Context = context.Background()
				var output bytes.Buffer
				...
				d = UpdateContext(ctx, DnsResourceData, client)
				Expect(d).To(BeNil())
				Expect(DnsResourceData.Get("guid")).To(Equal(new_guid))
	...
	Describe("Validating Resource Importer", func() {
		Context("Import A resource", func() {
			It("Resource:Dns ,Imports the resource", func() {
				var ctx context.Context = context.Background()
				var output bytes.Buffer
				...
				e := client.Start()
				Expect(e).To(BeNil())
				Importer.StateContext(ctx, DnsResourceData, client)
				//				Expect(d).To(BeNil())
