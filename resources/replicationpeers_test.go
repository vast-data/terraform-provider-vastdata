...
var _ = Describe(" ReplicationPeers", func() {
	...
	var ReplicationPeersResourceData *schema.ResourceData
	var model_json = `
                         {
   "guid": "string",
   "leading_vip": "string",
   ...
   "version": "string"
	...
	ReplicationPeersResource := resources.ResourceReplicationPeers()
	ReadContext = ReplicationPeersResource.ReadContext
	DeleteContext = ReplicationPeersResource.DeleteContext
	CreateContext = ReplicationPeersResource.CreateContext
	UpdateContext = ReplicationPeersResource.UpdateContext
	Importer = *ReplicationPeersResource.Importer
	//	ResourceSchema = ReplicationPeersResource.Schema

	BeforeEach(func() {
		ReplicationPeersResourceData = ReplicationPeersResource.TestResourceData()
		ReplicationPeersResourceData.SetId("100")
	...
	Describe("Validating Resource Read Context", func() {
		Context("Read Data into a ResourceData", func() {
			It("Resource:ReplicationPeers ,Reads Data", func() {
				...
				d := ReadContext(ctx, ReplicationPeersResourceData, client)
				Expect(d).To(BeNil())
				attributes := ReplicationPeersResourceData.State().Attributes
				for k, v := range z {
	...
	Describe("Validating Resource Delete Context", func() {
		Context("Delete A resource", func() {
			It("Resource:ReplicationPeers ,Deletes the resource", func() {
				var ctx context.Context = context.Background()
				var output bytes.Buffer
				...
				e := client.Start()
				Expect(e).To(BeNil())
				d := DeleteContext(ctx, ReplicationPeersResourceData, client)
				Expect(d).To(BeNil())
			})
		})
	},
	...
	Describe("Validating Resource Creation Context", func() {
		Context("Create A resource", func() {
			It("Resource:ReplicationPeers ,Creates the resource", func() {
				var ctx context.Context = context.Background()
				var output bytes.Buffer
				...
				resources.ResourceReplicationPeersReadStructIntoSchema(ctx, resource, ReplicationPeersResourceData)
				ReplicationPeersResourceData.SetId("100")
				d := CreateContext(ctx, ReplicationPeersResourceData, client)
				Expect(d).To(BeNil())
	...
	Describe("Validating Resource Update Context", func() {
		Context("Update A resource", func() {
			It("Resource:ReplicationPeers ,Update the resource", func() {
				var ctx context.Context = context.Background()
				var output bytes.Buffer
				...
				d = UpdateContext(ctx, ReplicationPeersResourceData, client)
				Expect(d).To(BeNil())
				Expect(ReplicationPeersResourceData.Get("guid")).To(Equal(new_guid))
	...
	Describe("Validating Resource Importer", func() {
		Context("Import A resource", func() {
			It("Resource:ReplicationPeers ,Imports the resource", func() {
				var ctx context.Context = context.Background()
				var output bytes.Buffer
				...
				resource := api_latest.ReplicationPeers{}
				json.Unmarshal([]byte(model_json), &resource)
				ReplicationPeersResourceData.SetId(guid)
				resource.Guid = guid
				b, err := json.Marshal(&resource)
				Expect(err).To(BeNil())
				...
				e := client.Start()
				Expect(e).To(BeNil())
				Importer.StateContext(ctx, ReplicationPeersResourceData, client)
				//				Expect(d).To(BeNil())
