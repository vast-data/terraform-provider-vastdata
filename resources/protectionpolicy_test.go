...
var _ = Describe(" ProtectionPolicy", func() {
	...
	var ProtectionPolicyResourceData *schema.ResourceData
	var model_json = `
                         {
   "clone_type": "string",
   "frames": [
   ...
   "url": "string"
	...
	ProtectionPolicyResource := resources.ResourceProtectionPolicy()
	ReadContext = ProtectionPolicyResource.ReadContext
	DeleteContext = ProtectionPolicyResource.DeleteContext
	CreateContext = ProtectionPolicyResource.CreateContext
	UpdateContext = ProtectionPolicyResource.UpdateContext
	Importer = *ProtectionPolicyResource.Importer
	//	ResourceSchema = ProtectionPolicyResource.Schema

	BeforeEach(func() {
		ProtectionPolicyResourceData = ProtectionPolicyResource.TestResourceData()
		ProtectionPolicyResourceData.SetId("100")
	...
	Describe("Validating Resource Read Context", func() {
		Context("Read Data into a ResourceData", func() {
			It("Resource:ProtectionPolicy ,Reads Data", func() {
				...
				d := ReadContext(ctx, ProtectionPolicyResourceData, client)
				Expect(d).To(BeNil())
				attributes := ProtectionPolicyResourceData.State().Attributes
				for k, v := range z {
	...
	Describe("Validating Resource Delete Context", func() {
		Context("Delete A resource", func() {
			It("Resource:ProtectionPolicy ,Deletes the resource", func() {
				var ctx context.Context = context.Background()
				var output bytes.Buffer
				...
				e := client.Start()
				Expect(e).To(BeNil())
				d := DeleteContext(ctx, ProtectionPolicyResourceData, client)
				Expect(d).To(BeNil())
			})
		})
	},
	...
	Describe("Validating Resource Creation Context", func() {
		Context("Create A resource", func() {
			It("Resource:ProtectionPolicy ,Creates the resource", func() {
				var ctx context.Context = context.Background()
				var output bytes.Buffer
				...
				resources.ResourceProtectionPolicyReadStructIntoSchema(ctx, resource, ProtectionPolicyResourceData)
				ProtectionPolicyResourceData.SetId("100")
				d := CreateContext(ctx, ProtectionPolicyResourceData, client)
				Expect(d).To(BeNil())
	...
	Describe("Validating Resource Update Context", func() {
		Context("Update A resource", func() {
			It("Resource:ProtectionPolicy ,Update the resource", func() {
				var ctx context.Context = context.Background()
				var output bytes.Buffer
				...
				d = UpdateContext(ctx, ProtectionPolicyResourceData, client)
				Expect(d).To(BeNil())
				Expect(ProtectionPolicyResourceData.Get("guid")).To(Equal(new_guid))
	...
	Describe("Validating Resource Importer", func() {
		Context("Import A resource", func() {
			It("Resource:ProtectionPolicy ,Imports the resource", func() {
				var ctx context.Context = context.Background()
				var output bytes.Buffer
				...
				resource := api_latest.ProtectionPolicy{}
				json.Unmarshal([]byte(model_json), &resource)
				ProtectionPolicyResourceData.SetId(guid)
				resource.Guid = guid
				b, err := json.Marshal(&resource)
				Expect(err).To(BeNil())
				...
				e := client.Start()
				Expect(e).To(BeNil())
				Importer.StateContext(ctx, ProtectionPolicyResourceData, client)
				//				Expect(d).To(BeNil())
