	...
	Describe("Validating Resource Read Context", func() {
		Context("Read Data into a ResourceData", func() {
			It("Resource:S3LifeCycleRule ,Reads Data", func() {
				...
				server.AppendHandlers(ghttp.CombineHandlers(
					ghttp.VerifyRequest("GET", "/api/s3lifecyclerules/100"),
	...
	Describe("Validating Resource Delete Context", func() {
		Context("Delete A resource", func() {
			It("Resource:S3LifeCycleRule ,Deletes the resource", func() {
				...
				ctx = tflogtest.RootLogger(ctx, &output)
				server.AppendHandlers(ghttp.CombineHandlers(
					ghttp.VerifyRequest("DELETE", "/api/s3lifecyclerules/100//"),
	...
	Describe("Validating Resource Creation Context", func() {
		Context("Create A resource", func() {
			It("Resource:S3LifeCycleRule ,Creates the resource", func() {
				...
				ctx = tflogtest.RootLogger(ctx, &output)
				server.AppendHandlers(ghttp.CombineHandlers(
					ghttp.VerifyRequest("POST", "/api/s3lifecyclerules/"),
					ghttp.RespondWith(200, model_json),
				),
				)
				server.AppendHandlers(ghttp.CombineHandlers(
					ghttp.VerifyRequest("GET", "/api/s3lifecyclerules/0"), //since this is a test http server and will not return id upon POST (creation) so json will use the zero value
					ghttp.RespondWith(200, model_json),
				),
				)
	...
	Describe("Validating Resource Update Context", func() {
		Context("Update A resource", func() {
			It("Resource:S3LifeCycleRule ,Update the resource", func() {
				...
				server.AppendHandlers(ghttp.CombineHandlers(
					ghttp.VerifyRequest("PATCH", "/api/s3lifecyclerules//0/"), //since this is a test http server and will not return id upon POST (creation) so json will use the zero value
					ghttp.RespondWith(200, model_json),
				),
				)
				...
				server.AppendHandlers(ghttp.CombineHandlers(
					ghttp.VerifyRequest("GET", "/api/s3lifecyclerules/0"), //the new_guid is returned and it should change the value of the resource
	...
	Describe("Validating Resource Importer", func() {
		Context("Import A resource", func() {
			It("Resource:S3LifeCycleRule ,Imports the resource", func() {
				...
				server.AppendHandlers(ghttp.CombineHandlers(
					ghttp.VerifyRequest("GET", "/api/s3lifecyclerules/", fmt.Sprintf("guid=%s", guid)), //since this is a test http server and will not return id upon POST (creation) so json will use the zero value
