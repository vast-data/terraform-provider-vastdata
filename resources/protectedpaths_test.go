...
var _ = Describe(" ProtectedPath", func() {
   ...
   "tenant_id": 100
	...
	ProtectedPathResource := resources.ResourceProtectedPath()
	ReadContext = ProtectedPathResource.ReadContext
	DeleteContext = ProtectedPathResource.DeleteContext
	CreateContext = ProtectedPathResource.CreateContext
	UpdateContext = ProtectedPathResource.UpdateContext
	Importer = *ProtectedPathResource.Importer
	//	ResourceSchema = ProtectedPathResource.Schema

	BeforeEach(func() {
		ProtectedPathResourceData = ProtectedPathResource.TestResourceData()
		ProtectedPathResourceData.SetId("100")
	...
	Describe("Validating Resource Read Context", func() {
		Context("Read Data into a ResourceData", func() {
			It("Resource:ProtectedPath ,Reads Data", func() {
				...
				d := ReadContext(ctx, ProtectedPathResourceData, client)
				Expect(d).To(BeNil())
				attributes := ProtectedPathResourceData.State().Attributes
				for k, v := range z {
	...
	Describe("Validating Resource Delete Context", func() {
		Context("Delete A resource", func() {
			It("Resource:ProtectedPath ,Deletes the resource", func() {
				var ctx context.Context = context.Background()
				var output bytes.Buffer
				...
				e := client.Start()
				Expect(e).To(BeNil())
				d := DeleteContext(ctx, ProtectedPathResourceData, client)
				Expect(d).To(BeNil())
			})
		})
	},
	...
	Describe("Validating Resource Creation Context", func() {
		Context("Create A resource", func() {
			It("Resource:ProtectedPath ,Creates the resource", func() {
				var ctx context.Context = context.Background()
				var output bytes.Buffer
				...
				resources.ResourceProtectedPathReadStructIntoSchema(ctx, resource, ProtectedPathResourceData)
				ProtectedPathResourceData.SetId("100")
				d := CreateContext(ctx, ProtectedPathResourceData, client)
				Expect(d).To(BeNil())
	...
	Describe("Validating Resource Update Context", func() {
		Context("Update A resource", func() {
			It("Resource:ProtectedPath ,Update the resource", func() {
				var ctx context.Context = context.Background()
				var output bytes.Buffer
				...
				d = UpdateContext(ctx, ProtectedPathResourceData, client)
				Expect(d).To(BeNil())
				Expect(ProtectedPathResourceData.Get("guid")).To(Equal(new_guid))
	...
	Describe("Validating Resource Importer", func() {
		Context("Import A resource", func() {
			It("Resource:ProtectedPath ,Imports the resource", func() {
				var ctx context.Context = context.Background()
				var output bytes.Buffer
				...
				resource := api_latest.ProtectedPath{}
				json.Unmarshal([]byte(model_json), &resource)
				ProtectedPathResourceData.SetId(guid)
				resource.Guid = guid
				b, err := json.Marshal(&resource)
				Expect(err).To(BeNil())
				...
				e := client.Start()
				Expect(e).To(BeNil())
				Importer.StateContext(ctx, ProtectedPathResourceData, client)
				//				Expect(d).To(BeNil())
