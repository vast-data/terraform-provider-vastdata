...
var _ = Describe(" User", func() {
	...
	var UserResourceData *schema.ResourceData
	var model_json = `
                         {
   "access_keys": [
      [
   ...
   "uid": 100
	...
	ReadContext = UserResource.ReadContext
	DeleteContext = UserResource.DeleteContext
	CreateContext = UserResource.CreateContext
	UpdateContext = UserResource.UpdateContext
	Importer = *UserResource.Importer
	//	ResourceSchema = UserResource.Schema

	BeforeEach(func() {
		UserResourceData = UserResource.TestResourceData()
		UserResourceData.SetId("100")
	...
	Describe("Validating Resource Read Context", func() {
		Context("Read Data into a ResourceData", func() {
			It("Resource:User ,Reads Data", func() {
				var ctx context.Context = context.Background()
				var output bytes.Buffer

				ctx = tflogtest.RootLogger(ctx, &output)
				server.AppendHandlers(ghttp.CombineHandlers(
					ghttp.VerifyRequest("GET", "/api/users/100"),
				...
				d := ReadContext(ctx, UserResourceData, client)
				Expect(d).To(BeNil())
				attributes := UserResourceData.State().Attributes
				for k, v := range z {
	...
	Describe("Validating Resource Delete Context", func() {
		Context("Delete A resource", func() {
			It("Resource:User ,Deletes the resource", func() {
				var ctx context.Context = context.Background()
				var output bytes.Buffer

				ctx = tflogtest.RootLogger(ctx, &output)
				server.AppendHandlers(ghttp.CombineHandlers(
					ghttp.VerifyRequest("DELETE", "/api/users/100//"),
					ghttp.RespondWith(200, "DELETED"),
				),
				)
				e := client.Start()
				Expect(e).To(BeNil())
				d := DeleteContext(ctx, UserResourceData, client)
				Expect(d).To(BeNil())
			})
		})
	},
	...
	Describe("Validating Resource Creation Context", func() {
		Context("Create A resource", func() {
			It("Resource:User ,Creates the resource", func() {
				var ctx context.Context = context.Background()
				var output bytes.Buffer
				...
				server.AppendHandlers(ghttp.CombineHandlers(
					ghttp.VerifyRequest("GET", "/api/users/0"), //since this is a test http server and will not return id upon POST (creation) so json will use the zero value
					ghttp.RespondWith(200, model_json),
				),
				)
				...
				resources.ResourceUserReadStructIntoSchema(ctx, resource, UserResourceData)
				UserResourceData.SetId("100")
				d := CreateContext(ctx, UserResourceData, client)
				Expect(d).To(BeNil())
	...
	Describe("Validating Resource Update Context", func() {
		Context("Update A resource", func() {
			It("Resource:User ,Update the resource", func() {
				var ctx context.Context = context.Background()
				var output bytes.Buffer
				...
				server.AppendHandlers(ghttp.CombineHandlers(
					ghttp.VerifyRequest("PATCH", "/api/users//0/"), //since this is a test http server and will not return id upon POST (creation) so json will use the zero value
					ghttp.RespondWith(200, model_json),
				),
				)
				...
				server.AppendHandlers(ghttp.CombineHandlers(
					ghttp.VerifyRequest("GET", "/api/users/0"), //the new_guid is returned and it should change the value of the resource
					ghttp.RespondWith(200, string(b)),
				),
				)

				d = UpdateContext(ctx, UserResourceData, client)
				Expect(d).To(BeNil())
				Expect(UserResourceData.Get("guid")).To(Equal(new_guid))
	...
	Describe("Validating Resource Importer", func() {
		Context("Import A resource", func() {
			It("Resource:User ,Imports the resource", func() {
				var ctx context.Context = context.Background()
				var output bytes.Buffer
				...
				resource := api_latest.User{}
				json.Unmarshal([]byte(model_json), &resource)
				UserResourceData.SetId(guid)
				resource.Guid = guid
				b, err := json.Marshal(&resource)
				Expect(err).To(BeNil())

				server.AppendHandlers(ghttp.CombineHandlers(
					ghttp.VerifyRequest("GET", "/api/users/", fmt.Sprintf("guid=%s", guid)), //since this is a test http server and will not return id upon POST (creation) so json will use the zero value
					ghttp.RespondWith(200, `[`+string(b)+`]`),
				),
				)
				...
				e := client.Start()
				Expect(e).To(BeNil())
				Importer.StateContext(ctx, UserResourceData, client)
				//				Expect(d).To(BeNil())
