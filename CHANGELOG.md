...
* **vastdata_nonlocal_user_key**: Custom `access_key` and `secret_key` values are not supported for non-local users. Keys are always generated by VMS. **Upgrade:** remove `access_key` and `secret_key` from existing `.tf` configurations before upgrading — Terraform rejects them as read-only attributes at plan time. Existing state is unaffected; keys remain available as computed outputs after refresh.
