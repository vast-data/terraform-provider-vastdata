...
page_title: "vastdata_quota_group Data Source - vastdata"
subcategory: ""
description: |-
  
---

# vastdata_quota_group (Data Source)
...
```terraform
data "vastdata_quota_group" "example" {
  name = "my-quota-group"
}
```
...
- `pretty_state` (String)
- `quotas_count` (Number)
- `quotas_ids` (List of Number) List of quota IDs assigned to this quota group.
- `refresh_user_quotas` (Boolean) Trigger field for refreshing user quota counters.
- `reset_grace_period` (Boolean) Trigger field for resetting the grace period countdown.
- `show_user_rules` (Boolean)
