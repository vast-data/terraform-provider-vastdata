...
- `ips_count` (Number) Number of IPs to allocate automatically. When set, the provider uses POST /vippools/allocate/ for creation and PATCH /vippools/{id}/reallocate/ for updates instead of the standard endpoints. Mutually exclusive with ip_ranges.
- `name` (String)
