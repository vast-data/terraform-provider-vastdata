...
- `get_views_count` (Boolean) Controls fetching of the "view_count" sub-resource (requires VAST >= 5.5.0). When unset or true the sub-resource is fetched automatically on supported clusters. Set to false to explicitly opt out.
...
- `title` (String)
- `url` (String)
- `view_count` (Attributes) (see [below for nested schema](#nestedatt--view_count))
...
<a id="nestedatt--view_count"></a>
### Nested Schema for `view_count`

Read-Only:

- `current_views_count` (Number) The number of views currently present in this tenant.
- `max_views` (Number) Maximum number of views allowed in this tenant.
