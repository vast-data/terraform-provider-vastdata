...
- `get_s3_true_ip_config` (Boolean) Controls fetching of the "s3_true_ip_config" sub-resource (requires VAST >= 5.5.0). When unset or true the sub-resource is fetched automatically on supported clusters. Set to false to explicitly opt out.
...
- `s3_true_client_ip_header` (String) True-client-IP header name for S3 requests.
