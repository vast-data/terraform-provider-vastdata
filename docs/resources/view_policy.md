...
To create a view policy named `vpolicy1`: 
...
- `protocols_audit` (Block List) (Valid for versions: 5.0.0,5.1.0,5.2.0) (see [nested schema](#nestedblock--protocols_audit) below)
...
- `use32bit_fileid` (Boolean) If `true`, VAST NFS server uses 32-bit file IDs.
...
- `vippool_permissions` (Block List) (Valid for versions: 5.1.0,5.2.0) A list of virtual IP pool permissions. This attribute conflicts with `vip_pools` and cannot be supplied together with it (see the [nested schema](#nestedblock--vippool_permissions) below).
...
- `modify_data` (Boolean) (Valid for versions: 5.1.0,5.2.0) Enables or disables logging of operations that modify data.
...
- `read_data_md` (Boolean) (Valid for versions: 5.1.0,5.2.0) Enables or disables logging of operations that read metadata.
...
- `vippool_permissions` (String) (Valid for versions: 5.1.0,5.2.0) Virtual IP pool permissions. Valid values: [RW RO]
...
- Import by GUID:
    ...
    To obtain the GUID, make an API call returning a list of all view policies with their IDs, names and GUIDs, for example:

        ```shell
        curl -sk -u admin https://<vms-ip>:443/api/v5/viewpolicies/ | jq '.[]|(.id,.name,.guid)'
        ```

- Import by policy name and tenant name. Enclose the argument in quotes to ensure proper handling of the pipe literal (|).

        ```shell
        terraform import vastdata_view_policy.example '<policy name>|<tenant name>'
        ```

    For example:

        ```shell
        terraform import vastdata_view_policy.archive 'mypolicy|default'
        ```
