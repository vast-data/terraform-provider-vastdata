...
resource "vastdata_administators_roles" "man_role1" {
  name = var.role_name
...
resource "vastdata_administators_managers" "manager1" {
  ...
  roles = [vastdata_administators_roles.man_role1.id]
  permissions_list = var.permissions_list
}

output tf_role {
  value = vastdata_administators_roles.man_role1
}

output tf_man {
  value = vastdata_administators_managers.manager1
  sensitive = true
}
