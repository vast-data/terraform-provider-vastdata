# Copyright (c) HashiCorp, Inc.

# To refer to a specific system, need to add `provider = vastdata.system<IDX>` where
# IDX is the system's index (in the order of appearance in Comet's commandline)

variable view_path {
    type = string
}

variable view_protocols {
    type = list(string)
}

variable view_policy_name {
    type = string
}

variable is_seamless {
    type = bool
}

variable max_retention_period {
    type = string
}

variable min_retention_period {
    type = string
}

variable files_retention_mode {
    type = string
}

variable default_retention_period {
    type = string
}

variable auto_commit {
    type = string
}

variable s3_object_ownership_rule {
    type = string
}

variable locking {
    type = bool
}

# Create a view with NFS & NFSv4 protocols
resource vastdata_view_policy viewpolicy_worm {
   name = var.view_policy_name
   flavor = "MIXED_LAST_WINS"
   use_auth_provider = "false"
   auth_source = "RPC"
   nfs_no_squash = ["10.0.0.1", "10.0.0.2"]
}

resource vastdata_view view1 {
  path = var.view_path
  policy_id = vastdata_view_policy.viewpolicy_worm.id
  create_dir = "true"
  protocols = var.view_protocols

  is_seamless = var.is_seamless
  max_retention_period = var.max_retention_period
  min_retention_period = var.min_retention_period
  files_retention_mode = var.files_retention_mode
  default_retention_period = var.default_retention_period
  auto_commit = var.auto_commit
  s3_object_ownership_rule = var.s3_object_ownership_rule
  locking = var.locking
}

output tf_view {
  value = vastdata_view.view1
}

output tf_view_view_policy {
  value = vastdata_view_policy.viewpolicy_worm
}