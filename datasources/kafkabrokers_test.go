...
var _ = Describe(" Kafkabroker", func() {
	...
	var KafkabrokerDataSourceData *schema.ResourceData
	var model_json = `
                         {
   "addresses": [
      {
   ...
   "tenant_id": 100
	...
	KafkabrokerDataSource := datasources.DataSourceKafkabroker()
	ReadContext = KafkabrokerDataSource.ReadContext

	BeforeEach(func() {
		KafkabrokerDataSourceData = KafkabrokerDataSource.TestResourceData()
		KafkabrokerDataSourceData.SetId("100")
	...
	Describe("Validating Datasource Read", func() {
		Context("Read A datasource", func() {
			It("Datasource:Kafkabroker ,reads", func() {
				...
				KafkabrokerDataSourceData.SetId(guid)
				resource.Guid = guid
				...
				KafkabrokerDataSourceData.Set("name", resource.Name)

				server.AppendHandlers(ghttp.CombineHandlers(
					ghttp.VerifyRequest("GET", "kafkabrokers", values.Encode()), //since this is a test http server and will not return id upon POST (creation) so json will use the zero value
				...
				e := client.Start()
				Expect(e).To(BeNil())
				d := ReadContext(ctx, KafkabrokerDataSourceData, client)
				Expect(d).To(BeNil())
				...
				attributes := KafkabrokerDataSourceData.State().Attributes
				for k, v := range z {
