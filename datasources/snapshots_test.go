...
var _ = Describe(" Snapshot", func() {
	...
	var SnapshotDataSourceData *schema.ResourceData
	var model_json = `
                         {
   ...
   "tenant_id": 100
	...
	SnapshotDataSource := datasources.DataSourceSnapshot()
	ReadContext = SnapshotDataSource.ReadContext

	BeforeEach(func() {
		SnapshotDataSourceData = SnapshotDataSource.TestResourceData()
		SnapshotDataSourceData.SetId("100")
	...
	Describe("Validating Datasource Read", func() {
		Context("Read A datasource", func() {
			It("Datasource:Snapshot ,reads", func() {
				...
				SnapshotDataSourceData.SetId(guid)
				resource.Guid = guid
				...
				SnapshotDataSourceData.Set("name", resource.Name)

				server.AppendHandlers(ghttp.CombineHandlers(
					ghttp.VerifyRequest("GET", "/api/snapshots/", values.Encode()), //since this is a test http server and will not return id upon POST (creation) so json will use the zero value
				...
				e := client.Start()
				Expect(e).To(BeNil())
				d := ReadContext(ctx, SnapshotDataSourceData, client)
				Expect(d).To(BeNil())
				...
				attributes := SnapshotDataSourceData.State().Attributes
				for k, v := range z {
