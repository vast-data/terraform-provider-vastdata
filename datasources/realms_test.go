...
var _ = Describe(" Realm", func() {
   ...
   "object_types": [
	...
	RealmDataSource := datasources.DataSourceRealm()
	ReadContext = RealmDataSource.ReadContext

	BeforeEach(func() {
		RealmDataSourceData = RealmDataSource.TestResourceData()
		RealmDataSourceData.SetId("100")
	...
	Describe("Validating Datasource Read", func() {
		Context("Read A datasource", func() {
			It("Datasource:Realm ,reads", func() {
				...
				RealmDataSourceData.SetId(guid)
				resource.Guid = guid
				...
				RealmDataSourceData.Set("name", resource.Name)

				server.AppendHandlers(ghttp.CombineHandlers(
					ghttp.VerifyRequest("GET", "realms", values.Encode()), //since this is a test http server and will not return id upon POST (creation) so json will use the zero value
				...
				e := client.Start()
				Expect(e).To(BeNil())
				d := ReadContext(ctx, RealmDataSourceData, client)
				Expect(d).To(BeNil())
				...
				attributes := RealmDataSourceData.State().Attributes
				for k, v := range z {
