...
var _ = Describe(" BlockHost", func() {
	...
	var BlockHostDataSourceData *schema.ResourceData
	var model_json = `
                         {
   "blockhost_tags": [
      "A",
   ...
   "tenant_id": 100
	...
	BlockHostDataSource := datasources.DataSourceBlockHost()
	ReadContext = BlockHostDataSource.ReadContext

	BeforeEach(func() {
		BlockHostDataSourceData = BlockHostDataSource.TestResourceData()
		BlockHostDataSourceData.SetId("100")
	...
	Describe("Validating Datasource Read", func() {
		Context("Read A datasource", func() {
			It("Datasource:BlockHost ,reads", func() {
				...
				BlockHostDataSourceData.SetId(guid)
				resource.Guid = guid
				...
				e := client.Start()
				Expect(e).To(BeNil())
				d := ReadContext(ctx, BlockHostDataSourceData, client)
				Expect(d).To(BeNil())
				...
				attributes := BlockHostDataSourceData.State().Attributes
				for k, v := range z {
