...
var _ = Describe(" Role", func() {
	...
	var RoleDataSourceData *schema.ResourceData
	var model_json = `
                         {
   "guid": "string",
   "ldap_groups": [
   ...
   "tenants": [
	...
	RoleDataSource := datasources.DataSourceRole()
	ReadContext = RoleDataSource.ReadContext

	BeforeEach(func() {
		RoleDataSourceData = RoleDataSource.TestResourceData()
		RoleDataSourceData.SetId("100")
	...
	Describe("Validating Datasource Read", func() {
		Context("Read A datasource", func() {
			It("Datasource:Role ,reads", func() {
				...
				RoleDataSourceData.Set("name", resource.Name)

				server.AppendHandlers(ghttp.CombineHandlers(
					ghttp.VerifyRequest("GET", "roles", values.Encode()), //since this is a test http server and will not return id upon POST (creation) so json will use the zero value
				...
				e := client.Start()
				Expect(e).To(BeNil())
				d := ReadContext(ctx, RoleDataSourceData, client)
				Expect(d).To(BeNil())
				...
				attributes := RoleDataSourceData.State().Attributes
				for k, v := range z {
