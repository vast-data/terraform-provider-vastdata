...
var _ = Describe(" ProtectionPolicy", func() {
	...
	var ProtectionPolicyDataSourceData *schema.ResourceData
	var model_json = `
                         {
   "clone_type": "string",
   "frames": [
   ...
   "url": "string"
	...
	ProtectionPolicyDataSource := datasources.DataSourceProtectionPolicy()
	ReadContext = ProtectionPolicyDataSource.ReadContext

	BeforeEach(func() {
		ProtectionPolicyDataSourceData = ProtectionPolicyDataSource.TestResourceData()
		ProtectionPolicyDataSourceData.SetId("100")
	...
	Describe("Validating Datasource Read", func() {
		Context("Read A datasource", func() {
			It("Datasource:ProtectionPolicy ,reads", func() {
				...
				ProtectionPolicyDataSourceData.SetId(guid)
				resource.Guid = guid
				...
				ProtectionPolicyDataSourceData.Set("name", resource.Name)

				server.AppendHandlers(ghttp.CombineHandlers(
					ghttp.VerifyRequest("GET", "/api/protectionpolicy/", values.Encode()), //since this is a test http server and will not return id upon POST (creation) so json will use the zero value
				...
				e := client.Start()
				Expect(e).To(BeNil())
				d := ReadContext(ctx, ProtectionPolicyDataSourceData, client)
				Expect(d).To(BeNil())
				...
				attributes := ProtectionPolicyDataSourceData.State().Attributes
				for k, v := range z {
