...
var _ = Describe(" Group", func() {
	...
	var GroupDataSourceData *schema.ResourceData
	var model_json = `
                         {
   "gid": 100,
   "guid": "string",
   ...
   "sid": "string"
	...
	GroupDataSource := datasources.DataSourceGroup()
	ReadContext = GroupDataSource.ReadContext

	BeforeEach(func() {
		GroupDataSourceData = GroupDataSource.TestResourceData()
		GroupDataSourceData.SetId("100")
	...
	Describe("Validating Datasource Read", func() {
		Context("Read A datasource", func() {
			It("Datasource:Group ,reads", func() {
				...
				GroupDataSourceData.SetId(guid)
				resource.Guid = guid
				...
				e := client.Start()
				Expect(e).To(BeNil())
				d := ReadContext(ctx, GroupDataSourceData, client)
				Expect(d).To(BeNil())
				...
				attributes := GroupDataSourceData.State().Attributes
				for k, v := range z {
